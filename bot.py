    ...
    try:
        ...
        if chat_id in fetch_cache:
            ...
            if current_time - last_fetch < MIN_FETCH_INTERVAL:
                ...
                # Return cached prayer times from database if available
        ...
        logger.info(f"Fetching prayer times for {city}, {country} (method: {method})")
...
def send_azan_notification(chat_id: int, prayer_name: str, prayer_time: str) -> bool:
    """
    Send azan notification to a group.
    """
    try:
        prayer_names_arabic = {
            'Fajr': 'الفجر',
...
استخدم زر 📋 لاحقاً لتسجيل صلاتك!
        ...
        logger.error(f"Error sending azan notification: {e}", exc_info=True)
        return False
            ...
            for group in groups:
                try:
                    ...
                    if not prayer_times_data:
                        logger.info(f"No prayer times for group {chat_id} today, fetching...")
                        prayer_times = fetch_and_save_prayer_times(chat_id)
                        if prayer_times:
                            prayer_times_data = {'fajr_time': prayer_times['Fajr'],
...
def mark_content_as_sent(chat_id: int, content_type: str, date: str):
    ...
    try:
        ...
        logger.error(f"Error in mark_content_as_sent: {e}", exc_info=True)
...
def content_scheduler_loop():
    ...
    Runs in a separate thread and checks scheduled content times.
    """
        ...
        try:
            ...
            for group in groups:
                try:
                    ...
                    # Check each scheduled content type
                    for content_type, schedule_info in schedule_config.items():
                        try:
                            ...
                            # Get scheduled times for this content type
                            scheduled_times = schedule_info.get('times', [])

                            for scheduled_time in scheduled_times:
                                try:
                                    # Parse scheduled time
                                    sched_hour, sched_minute = parse_time(scheduled_time)

                                    # Check if current time is within the window
                                    if time_within_window(
                                        (current_hour, current_minute),
                                        (sched_hour, sched_minute),
                                        window_minutes=CONTENT_WINDOW_MINUTES
                                    ):
                                        # Check if content already sent today
                                    ...
                                    logger.error(f"Error processing schedule time {scheduled_time}: {e}")

                        except Exception as e:
                            logger.error(f"Error processing content type {content_type}: {e}")

                except Exception as e:
                    logger.error(f"Error processing group {group.get('chat_id', 'unknown')} in content scheduler: {e}")

        except Exception as e:
            logger.error(f"Error in content scheduler loop: {e}", exc_info=True)

    logger.info("Content scheduler loop stopped gracefully")
